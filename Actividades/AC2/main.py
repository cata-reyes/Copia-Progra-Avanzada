import collections
from os.path import join
from utilidades import Anime  # IMPORTANT: Debes utilizar esta nametupled


#####################################
#       Parte I - Cargar datos      #
#####################################
def cargar_animes(ruta_archivo: str) -> list:
    file=open(ruta_archivo,"r",encoding="utf-8")
    linea=file.readline()
    lista_animes=[]
    while linea != "":
        lista_anime=linea.split(",")
        generos=lista_anime[-1].split(";")
        generos[-1]=generos[-1].strip("\n")
        lista_anime[-1]=set(generos)
        tupla_animes=collections.namedtuple("Anime",["nombre","capitulos","puntaje","estreno","estudio","generos"])
        anime_i=tupla_animes(lista_anime[0],int(lista_anime[1]),int(lista_anime[2]),int(lista_anime[3]),lista_anime[4],lista_anime[5])
        lista_animes.append(anime_i)
        linea=file.readline()
    return lista_animes


#####################################
#        Parte II - Consultas       #
#####################################
def animes_por_estreno(animes: list) -> dict:
    lista_fechas=[]
    for i in range (len(animes)):
        lista_fechas.append(animes[i].estreno)
    set_fechas=set(lista_fechas)
    contador=0
    diccionario={}
    for i in set_fechas:
        lista=[]
        for j in range(len(animes)):
            if animes[j].estreno == set_fechas:
                lista.append(animes[j].nombre)
        diccionario_extra={set_fechas:lista}
        diccionario.update(diccionario_extra)

    return diccionario


def descartar_animes(generos_descartados: set, animes: list) -> list:
    # TODO: Completar
    return


def resumen_animes_por_ver(*animes: Anime) -> dict:
    # TODO: Completar
    return


def estudios_con_genero(genero: str, **estudios: dict) -> list:
    # TODO: Completar
    return


if __name__ == "__main__":
    #####################################
    #       Parte I - Cargar datos      #
    #####################################
    animes = cargar_animes(join("data", "ejemplo.chan"))
    indice = 0
    for anime in animes:
        print(f"{indice} - {anime}")
        indice += 1

    #####################################
    #        Parte II - Consultas       #
    #####################################
    # Solo se usará los 2 animes del enunciado.
    datos = [
        Anime(
            nombre="Hunter x Hunter",
            capitulos=62,
            puntaje=9,
            estreno=1999,
            estudio="Nippon Animation",
            generos={"Aventura", "Comedia", "Shonen", "Acción"},
        ),
        Anime(
            nombre="Sakura Card Captor",
            capitulos=70,
            puntaje=10,
            estreno=1998,
            estudio="Madhouse",
            generos={"Shoujo", "Comedia", "Romance", "Acción"},
        ),
    ]

    # animes_por_estreno
    estrenos = animes_por_estreno(datos)
    print(estrenos)

    # descartar_animes
    animes = descartar_animes({"Comedia", "Horror"}, datos)
    print(animes)

    # resumen_animes_por_ver
    resumen = resumen_animes_por_ver(datos[0], datos[1])
    print(resumen)

    # estudios_con_genero
    estudios = estudios_con_genero(
        "Shonen",
        Nippon_Animation=[datos[0]],
        Madhouse=[datos[1]],
    )
    print(estudios)




