...
def cargar_sismos(path_archivo: str) -> pd.DataFrame:
    ...
    with open(path_archivo, encoding= 'utf-8') as archivo:
        ...
        linea = linea.strip("\n")
        linea = linea.split(",")
        datos = {titulo:[] for titulo in linea}
        ...
        for linea in archivo:
            ...
            i = 0
            for titulo in datos:
                datos[titulo].append(linea[i])
                i += 1
    df_sismo = pd.DataFrame(datos)
    return df_sismo   
    # TODO: Parte I
...
def limpiar_header_corrompido(header: str) -> str:
    ...
    la corrupción. 
    '''
    header = re.sub('\d', "", header)
    header = re.sub('[!#$%&?*+]', "" , header)
    header = re.sub('[A-Z]+[A-Z]', "", header)
    return header
    # TODO: Parte IV
