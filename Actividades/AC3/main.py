...
class NodoCliente:
    ...
    def __str__(self) -> str:
        return f'C({self.identificador}) -> {self.siguiente}'
...
class SistemaColas:
    ...
    def agregar_persona(self, preferencial: bool) -> None:
        nuevo_cliente=NodoCliente(preferencial)
        if preferencial == True:
            if self.cola_preferencial == None:
                self.cola_preferencial=nuevo_cliente
            else:
                self.cola_preferencial.agregar_nodo(nuevo_cliente)
        else:
            if self.cola_normal == None:
                self.cola_normal=nuevo_cliente
            else:
                self.cola_normal.agregar_nodo(nuevo_cliente)


    def __len__(self) -> int:
        contador=0
        if self.cola_preferencial != None:
            contador+=len(self.cola_preferencial)
        if self.cola_normal != None:
            contador+=self.cola_normal.__len__()
        return contador
